...
def center_surround_diff(gauss_pyr):
    ...
    for c in range(2, 5):
        ...
        for s in range(3, 5):
            surround = cv2.resize(gauss_pyr[c + s], size, interpolation=cv2.INTER_LINEAR)
...
def normalize_map(feature_map):
    ...
    avg_local_maximum = compute_average_local_maxima(simply_normalized)
...
def get_intensity_conspicuity_map(intensity):
    ...
    norm_feature_maps = [
        cv2.resize(m, shape[::-1], interpolation=cv2.INTER_LINEAR) for m in norm_feature_maps
    ]
...
def get_color_conspicuity_map(image):
    ...
    rg_color_space_maps = get_color_space_feature_maps(red_gauss_pyr, green_gauss_pyr)
    by_color_space_maps = get_color_space_feature_maps(blue_gauss_pyr, yellow_gauss_pyr)

    # normalize feature maps
    rg_color_space_maps = [normalize_map(m) for m in rg_color_space_maps]
    by_color_space_maps = [normalize_map(m) for m in by_color_space_maps]
...
def get_color_channels(image):
    ...
    for y in range(image.shape[0]):
        for x in range(image.shape[1]):
            if image[y][x].any() > 0:
                src[y][x] = np.float32(np.float32(image[y][x]) / np.float32(max_int))
...
def get_color_space_feature_maps(c1_pyr, c2_pyr):
    ...
    for c in range(2, 5):
        ...
        for s in range(3, 5):
            ...
            res_surround = center - cv2.resize(surround, size, interpolation=cv2.INTER_LINEAR)
...
def get_conspicuity_from_color_spaces(rg_space, by_space, shape):
    ...
    # NOTE: in 1998 paper they use scale 4 for the conspicuity maps, here I use scale 0
    rg_space = [cv2.resize(m, shape[::-1], interpolation=cv2.INTER_LINEAR) for m in rg_space]
    by_space = [cv2.resize(m, shape[::-1], interpolation=cv2.INTER_LINEAR) for m in by_space]
    for cs_index in range(6):
...
def get_conspicuity_from_orienation_feature_maps(orient_feature_maps, shape, debug=False):
    ...
    for orientation in orient_feature_maps:
        ...
        for feature_map in orientation:
            ...
            resized = cv2.resize(normalized, shape[::-1], interpolation=cv2.INTER_LINEAR)
...
def get_orientation_conspicuity_map(intensity, debug=False):
    ...
    oriented_intensities = [cv2.filter2D(intensity, cv2.CV_32F, kernel) for kernel in filters]
    ...
    cs_differences = [center_surround_diff(oriented_pyr) for oriented_pyr in intensity_pyramids]
...
def get_spatial_saliency_map_I(image):
    intensity = get_intensity(image)
    intensity_conspicuity = get_intensity_conspicuity_map(intensity)
    # color_conspicuity = get_color_conspicuity_map(image)
    # orientation_conspicuity = get_orientation_conspicuity_map(intensity)
    ...
    # c_intensity = normalize_map(color_conspicuity)
    # o_intensity = normalize_map(orientation_conspicuity)
    saliency_map = n_intensity  # (n_intensity + c_intensity + o_intensity) * (1 / 3)

    return saliency_map


def get_spatial_saliency_map_C(image):
    # intensity = get_intensity(image)
    # intensity_conspicuity = get_intensity_conspicuity_map(intensity)
    color_conspicuity = get_color_conspicuity_map(image)
    # orientation_conspicuity = get_orientation_conspicuity_map(intensity)
    ...
    c_intensity = normalize_map(color_conspicuity)
    # o_intensity = normalize_map(orientation_conspicuity)
    saliency_map = c_intensity  # (n_intensity + c_intensity + o_intensity) * (1 / 3)

    return saliency_map


def get_spatial_saliency_map_O(image):
    intensity = get_intensity(image)
    # intensity_conspicuity = get_intensity_conspicuity_map(intensity)
    # color_conspicuity = get_color_conspicuity_map(image)
    orientation_conspicuity = get_orientation_conspicuity_map(intensity)
    ...
    # c_intensity = normalize_map(color_conspicuity)
    o_intensity = normalize_map(orientation_conspicuity)
    saliency_map = o_intensity  # (n_intensity + c_intensity + o_intensity) * (1 / 3)
